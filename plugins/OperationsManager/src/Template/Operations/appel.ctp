...
<table width="100%">
    <thead>
    <tr>
        <th width="45%"><?= __('Expéditeur') ?></th>
        <th width="10%"></th>
        <th width="45%"><?= __('Destinataire') ?></th>
    </tr>
    </thead>
    <tbody>
    <tr>
        ...
        <td>
            <?= $operation->client->nom_complet ?><br/>
            <?= $operation->client->adresse1 ?><br/>
            <?= $operation->client->adresse2 ?><br/>
            <?= $operation->client->code_postal . ' ' . $operation->client->ville ?>
        </td>
    </tr>
    </tbody>
</table>
...
<table width="100%">
    ...
    <tr>
        <th width="100%" style="border: 0.1mm solid #888888;">
            ...
            du <?= $this->Time->format($operation->created, 'dd/MM/yyyy') ?> - Pour la période
            du <?= $this->Time->format($operation->debut_periode, 'dd/MM/yyyy') ?>
            au <?= $this->Time->format($operation->fin_periode, 'dd/MM/yyyy') ?>
        </th>
    </tr>
    </thead>
    <tr>
        <td width="100%" style="border: 0.1mm solid #888888;">
            En votre aimable réglement avant le <?= $this->Time->format($operation->date_echeance, 'dd/MM/yyyy') ?>
        </td>
    </tr>
</table>
<br/>
<table width="100%">
    ...
    <?php endforeach ?>
    <tr>
        <td class="totals" align="right" colspan="<?= $operation->bail->societe->tva ? '4' : '2' ?>"><?= __('Total € TTC à régler') ?></td>
        <td align="right" class="totals"><?= number_format($operation->total_ttc, 2, ",", " ") ?>€</td>
    </tr>
    </tbody>
</table>
<?php if (!empty($operation->commentairepublic)): ?>
    <table width="100%">
        <tbody>
        <tr>
            <td><?= ($operation->commentairepublic) ?></td>
        </tr>
        </tbody>
    </table>
<?php endif; ?>
