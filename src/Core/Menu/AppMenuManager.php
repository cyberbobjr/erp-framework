    ...
    class AppMenuManager
    {
        ...
        public static function getInstance()
        {
            if (empty(self::$instance)) {
                self::$instance = new AppMenuManager();
            }
            return self::$instance;
        }

        public function addMenu(String $pluginName, AppMenu $menu, $submenus, $position = 'left')
        {
            if (!is_a($menu, AppMenu::class)) {
            ...
            $newMenu = ['icon'     => $menu['icon'],
                        ...
                        'url'      => $menu['url'],
                        'order'    => $menu['order'],
                        'submenus' => $this->_validateSubMenu($submenus)];
        ...
        public function addSubmenu(string $menuName, string $position, array $array)
        {
            if (!isset($this->_menu[$position])) {
                $this->_menu[$position] = [$menuName => [[
                                                             'label'    => $menuName,
                                                             'submenus' => []]]];
            } elseif (!isset($this->_menu[$position][$menuName])) {
                $this->_menu[$position][$menuName] = [['label'    => $menuName,
                                                       'submenus' => []]];
